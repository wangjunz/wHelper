...
|  说明 | 方法名  |  参数  | 返回值示例 ｜
|  ----  | ----  |  ----  |  ----  |
| 1、获取当前日期 | getToday |  seperator：分隔符，不必填，默认’-‘  |  ----  |
2、获取当前日期时间 | getTodayTime |  seperator：分隔符，不必填，默认’-‘  |  ----  |
3、获取星期几 | getWeekDay |  dateStr：日期字符串，不必填，默认今天  |  ----  |
4、获取时间戳 | getCurrentTime |  -  |  ----  |
5、获取昨天 | getYesterday |  -  |  ----  |
7、获取明天 | getTomorrow |  -  |  ----  |
8、获取几天前/几天后的日期 | getData |  day：前后移动的天数（类型为正整数和负整数）；seperator：分隔符，不必填，默认’-‘  |  ----  |
10、获取最近七天的日期(默认倒序) | getLast7Day |  orderBy：排序方式（desc：倒序，asc：顺序），不必填，默认’desc‘  |  ----  |
12、计算两个日期相隔天数 | dateDiff | dateStr1,dateStr2：日期字符串，必填 |  ----  |
13、计算两个时间差，返回天小时分钟 | timeDiff | datetimeStr1,datetimeStr2：日期时间字符串，必填  |  ----  |
14、获得上周周一和周日的日期 | getLastWeekData |  -  |  ----  |
15、获得本周周一和周日的日期 | getThisWeekData |  -  |  ----  |
16、时间戳转换为几秒/几分/几小时/几天前 | getFriendlyDate |  timestamp：时间戳，必填  |  ----  |
...
<template>
	<view class="content_container">
		<uni-forms ref="baseForm" :modelValue="baseFormData">
			...
			<uni-forms-item label="8、获取5天前"><view class="v__value">{{getData5s}}</view></uni-forms-item>
			<uni-forms-item label="9、获取10天后"><view class="v__value">{{getData10p}}</view></uni-forms-item>
			...
			<view class="uni-forms-item is-direction-left" style="display: flex;position: relative;">
				 ...
				 <view class="v__value">
					 <view style="color:#333;">2023-09-01 15:07:12</view>
...
<script>
	...
	export default {
		...
		computed:{
			...
			getData5s(){
				return dateUtil.getData(-5)
			},
			getData10p(){
				return dateUtil.getData(10)
			},
			getLast7Day(){
			...
			getFriendlyDate(){
				var date = new Date('2023-09-01 15:07:12');
